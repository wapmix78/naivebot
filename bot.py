...
class NaivePanelClient:
    ...
    def _sess(self) -> httpx.AsyncClient:
        if self._session is None or self._session.is_closed:
            self._session = httpx.AsyncClient(verify=PANEL_VERIFY_SSL, timeout=10.0, cookies=self._jar)
            if not PANEL_VERIFY_SSL:
                logger.warning("⚠️ SSL-проверка панели ОТКЛЮЧЕНА (PANEL_VERIFY_SSL=false). MITM-уязвимость!")
    ...
    @property
    def _sem_(self) -> asyncio.Semaphore:
        if self._sem is None: self._sem = asyncio.Semaphore(10)
        return self._sem

    async def login(self) -> bool:
        async with self._lock_:
            try:
                r = await self._sess().post(
                ...
                if r.status_code == 200 and r.json().get("success"):
                    ...
                    self._state    = "CLOSED"
                    self.last_error = ""
                    logger.info("✅ Панель авторизована")
                    return True
                self.last_error = f"HTTP {r.status_code}: {r.text[:100]}"
            except httpx.ConnectError as e:
                self.last_error = f"ConnectError: {e}"
                logger.warning(f"Panel connect failed: {e}")
            except httpx.TimeoutException:
                self.last_error = "Timeout (панель не отвечает)"
                logger.warning("Panel login timeout")
            except Exception as e:
                self.last_error = str(e)
                logger.warning(f"Panel login error: {e}")
            self.logged_in = False
            return False

    async def _request(self, method: str, path: str, **kwargs):
        if self._state == "OPEN" and time.time() - self._opened_at < 60:
            return None
        if not self.logged_in and not await self.login():
            return None
        for attempt in range(2):
            try:
                async with self._sem_:
                    r = await getattr(self._sess(), method)(f"{PANEL_URL}{path}", **kwargs)
                if r.status_code == 401 and attempt == 0:
                    self.logged_in = False
                    if await self.login(): continue
                # FIX: принимаем 200 и 201 (Created) как успех
                if r.status_code in (200, 201):
                    ...
                    try:
                        ...
                        # Панель вернула не-JSON (пустой ответ) — считаем успехом
                        return {"success": True}
                # Логируем реальное тело ответа для диагностики
                body = ""
                try:
                    body = r.text[:300]
                except Exception:
                ...
                logger.warning(f"Panel {method.upper()} {path} → {r.status_code}: {body}")
            except Exception as e:
                self.last_error = str(e)
                logger.warning(f"Panel request error [{method} {path}]: {e}")
                if attempt == 0: await self.login()
        self._failures += 1
        if self._failures >= 5:
            self._state     = "OPEN"
            self._opened_at = time.time()
            logger.warning("⚠️ Panel circuit breaker OPEN")
        return None

    async def ping(self) -> tuple[bool, str]:
        """
        Диагностика подключения к панели.
        HEAD-запрос на корневой URL — не GET /api/login (POST-эндпоинт),
        иначе GET зависает и даёт ложный Timeout при рабочей панели.
        """
        for path in ["/", "/api/login"]:
            try:
                r = await self._sess().head(f"{PANEL_URL}{path}", timeout=5.0)
                ...
                return False, str(e)
        return False, "Таймаут — панель не отвечает"

    async def add_user(self, username: str, password: str) -> bool:
        """
        Добавить пользователя в NaiveProxy панель.
        Эндпоинт: POST /api/proxy-users/add (naiveproxy-panel).
        """
        data = await self._request("post", "/api/proxy-users/add",
        ...
        if data is not None:
            if isinstance(data, dict):
                if data.get("success"):
                    ...
                    return True
                # Пользователь уже существует — считаем успехом
                msg = data.get("message", "")
                if "уже существует" in msg or "already exist" in msg.lower():
                    logger.info(f"✅ Пользователь {username} уже есть в панели — OK")
                    return True
                self.last_error = str(data)
            else:
                logger.info(f"✅ Пользователь {username} добавлен (non-dict ответ)")
                return True
        logger.error(f"❌ Не удалось добавить {username}. Последняя ошибка: {self.last_error}")
        return False

    async def delete_user(self, username: str) -> bool:
        attempts = [
            ("delete", f"/api/proxy-users/{username}", {}),
            ("delete", f"/api/users/{username}",       {}),
            ("post",   "/api/proxy-users/delete",      {"username": username}),
        ]
        for method, path, body in attempts:
            ...
            if data is not None:
                if isinstance(data, dict):
                    if (data.get("success") or data.get("ok") or
                            data.get("status") in ("ok", "deleted") or not data):
                        return True
                else:
                    return True
        return False
    ...
    async def list_users(self) -> List[dict]:
        data = await self._request("get", "/api/proxy-users")
